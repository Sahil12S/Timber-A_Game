...
int main()
{
	...
	*/
	...
	spriteBackground.scale(Vector2f(0.75, 0.7));
	...
	for (int i = 0; i < NUM_CLOUDS; ++i) {
		...
		spriteCloud[i].scale(Vector2f(0.7, 0.7));
	...
	*/
	...
	fpsText.setPosition(1000, 20);

	// Background for the text
	RectangleShape rect1;
	rect1.setFillColor(sf::Color(0, 0, 0, 150));
	rect1.setSize(Vector2f(450, 120));
	rect1.setPosition(0, 10);

	RectangleShape rect2;
	rect2.setFillColor(sf::Color(0, 0, 0, 150));
	rect2.setSize(Vector2f(450, 120));
	rect2.setPosition(970, 10);
	...
	spritePlayer.setPosition(400, 600);
	...
	spriteAxe.setPosition(450, 620);

	// Line the axe up with the tree.
	const float AXE_POSITION_LEFT = 550;
	const float AXE_POSITION_RIGHT = 730;
	...
	while (window.isOpen()) {
		...
		*/
		...
		if (Keyboard::isKeyPressed(Keyboard::Return)) {
			...
			spriteRIP.setPosition(500, 1000);

			// Move the player into position
			spritePlayer.setPosition(480, 550);
			acceptInput = true;
		}
		...
		if (acceptInput) {
			...
			if (Keyboard::isKeyPressed(Keyboard::Right)) {
				...
				spritePlayer.setPosition(810, 550);

				// Update the branches
				updateBranches(score);

				// set the log flying to the left
				spriteLog.setPosition(600, 550);
				logSpeedX = -5000;
				logActive = true;
		...
		if (!paused) {
			...
			for (int i = 0; i < NUM_CLOUDS; ++i) {
				if (!cloudActive[i]) {
					...
					srand((int)time(0) * 10);
					float height = (rand() % (150 * (i + 1))) - 50 * i;
					...
					if (spriteCloud[i].getPosition().x > 1400) {
			...
			*/
			//  Draw score and frame rate once every 100 frames
			lastDrawn++;
			if (lastDrawn == 50) {
			...
			if (branchPositions[5] == playerSide) {
				...
				// Draw the gravestone
				spriteRIP.setPosition(500, 600);

				// hide the player
				spritePlayer.setPosition(2000, 550);
